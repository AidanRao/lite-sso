...
func TestAdminOAuthClients_CreateAndUpdate(t *testing.T) {
	...
	createBody := `{"name":"订单系统","client_id":"order-app","client_secret":"secret-1","redirect_uris":["https://order.example.com/callback"],"logout_uris":["https://order.example.com/logout"]}`
	...
	updateBody := `{"name":"订单中心","client_id":"order-center","redirect_uris":["https://order.example.com/oauth/callback"],"logout_uris":[]}`
	...
	if client.RedirectURIs != `["https://order.example.com/oauth/callback"]` {
		t.Fatalf("unexpected redirect uris: %s", client.RedirectURIs)
	}
}
